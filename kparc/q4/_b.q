...
\t select max price by sym,ex from trade where date=d,sym in S
\t select sum size by sym,time.hh from trade where date=d,sym in S
\t select from aj[`time;select time,price from trade where date=d,sym=`QQQ;select time,bid from quote where date=d,sym=`QQQ]where price<bid
\\
