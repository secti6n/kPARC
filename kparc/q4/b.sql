...
explain(analyze) select sym,last(bid order by time) from quote natural join S group by sym;
explain(analyze) select distinct on(sym)sym,time,last_value(bid) over(partition by sym order by time)from quote natural join S;
explain(analyze) select sym,bid from(select sym,bid,row_number()over(partition by sym order by time desc)as row from quote natural join S)q where row=1;
